#!/usr/bin/env python3
"""
Test runner for the runtime_interface_wam plugin.
...
Usage:
    ...
    python3 tests/run_tests.py -m e2e -v          # Interface + engine + dummy provider
    python3 tests/run_tests.py unit/test_metering.py -v
"""
import os
import sys
import types
...
def install_pylon_stubs():
    ...
    tools.auth = types.SimpleNamespace(current_user=lambda: {"id": None})
    # Registered by the projects plugin; utils/metering.py reads the one key off it.
    tools.project_constants = {'PROJECT_USER_NAME_PREFIX': ':system:project:'}
...
if __name__ == '__main__':
    ...
    # The plugin is imported as the package `runtime_interface_wam`, so relative imports resolve
